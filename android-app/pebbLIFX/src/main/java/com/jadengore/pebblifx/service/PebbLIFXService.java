...
public class PebbLIFXService extends Service {
    ...
    private final static int DEFAULT_KELVIN = 3500;
	...
	public void color (int target, int r, int g, int b) {
        ...
        LFXHSBKColor result = LFXHSBKColor.getColor(hsvResult[0],hsvResult[1],hsvResult[2], 3500);
        if (target == 0) {
            LFXHSBKColor color = localNetworkContext.getAllLightsCollection().getColor();
			localNetworkContext.getAllLightsCollection().setColor(result);
		} else {
			bulbList.get(target - 1).setColor(result);
		}
        Log.i("PebbLIFXService", "New HSBKColor" + result.toString());
		ack();
	}
